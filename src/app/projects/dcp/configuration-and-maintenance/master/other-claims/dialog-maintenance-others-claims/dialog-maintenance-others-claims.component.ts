...
export class DialogMaintenanceOthersClaimsComponent implements OnInit {
  ...
  loadFormEngineModel():void{
    if(this.data.option=='new'){
      ...
      const estate = this.data.reclamable.estado==1? true:false;
      this.formGroup = new FormGroup({
        codigo: new FormControl(this.data.reclamable.codigo,[Validators.required]),
        descripcion: new FormControl(this.data.reclamable.descripcion,[Validators.required]),
  ...
  onSaveOtherClaim():void{
    if(this.formGroup.valid){
      ...
      if(this.data.option=='new'){
        ...
        this.configurationAndMaintenanceService.maintenanceOtrosReclamables(request).subscribe(resp=>{
          if(resp.success){
        ...
        const request = { id:this.data.reclamable.id , activo:true , ...this.formGroup.value };
        this.configurationAndMaintenanceService.maintenanceOtrosReclamables(request).subscribe(resp=>{
          console.log(resp);
          if(resp.success){
            this.dialogRef.close(true);
